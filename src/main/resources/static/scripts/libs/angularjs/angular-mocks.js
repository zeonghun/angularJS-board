 ...
 *         expect($log.logger.logs).toEqual([[1], [2], [3]]);
 *       });
 *     });
 *   });
 * ```
...
angular.mock.$LogProvider = function() {
  ...
  this.$get = function() {
    var $log = {
      log: function() { $log.logger.logs.push(concat([], arguments, 0)); },
    ...
    $log.reset = function() {
       ...
       * ```js
       * $log.log('Some Log');
       * var first = $log.logger.logs.unshift();
       * ```
       */
      $log.logger.logs = [];
      /**
  ...
  function serializeScope(scope, offset) {
    ...
    for (var key in scope) {
      if (Object.prototype.hasOwnProperty.call(scope, key) && !key.match(/^(\$|this)/)) {
        logger.push('  ' + key + ': ' + angular.toJson(scope[key]));
      }
    }
    var child = scope.$$childHead;
    while (child) {
      logger.push(serializeScope(child, offset + '  '));
